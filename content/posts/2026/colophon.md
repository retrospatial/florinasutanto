...
Now that I have components that make server-side API calls, I can't use GitHub Pages anymore since it can only deploy static sites. Luckily I'm used to using Vercel at work so it was an easy switch––just had to connect the repo, add my API keys to the environment variables, and that's it! I'm not too worried about the bandwidth limits on the free tier right now, but as my site grows (especially with images) I might have to look into setting up a CDN.
